...
KEGG_circ<- function(project,DEG,n, hu_man, human_db) {
  ...
  # 2. 计算富集结果
  #2. 2 进行KEGG通路的GSEA
  KEGG_gseresult <- gseKEGG(geneList,organism = hu_man,exponent = 1.5, nPerm = 1000000,minGSSize = 5,maxGSSize = 1000, pvalueCutoff =1)
  ...
  if(nrow(KEGG_gseresult@result)>n){
    n1=n
  } else {
    n1=nrow(KEGG_gseresult@result)
  }
 
  Go1<-KEGG_gseresult@result[1:n1,c(1,1,2,8,11)]
