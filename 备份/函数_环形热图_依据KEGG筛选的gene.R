...
heatmap_KEGG<- function(project,EXP,DEG,hu_man, human_db,df) {
  if (missing(hu_man)) 
    hu_man <- "human"
  if (missing(df)) 
    df <- DEG
...
circ2=KEGG_circ(project ,DEG=df,n=100, hu_man=hu_man, human_db=human_db)
...
# 将原始行名替换为新的行名序列
colnames(exp1) <- paste0("sample",make.unique(rep(letters, length.out = ncol(exp1))))
...
pdf(paste0(project,"/",project, "-","heatmap_KEGG环形热图.pdf"), width = 10, height = 8)
...
p3=ggtree(tree,branch.length = "none", layout = "circular",
  ...
  geom_tiplab(offset=6.6,show.legend=FALSE,size=1.8,
  ...
  geom_fruit(data=df4,geom=geom_point, mapping=aes(y=Gene, x=logFC,color=-log10(pvalue)), size=1.5,
             pwidth=0.22,offset = 0.31,axis.params=list(axis="x"  #添加x轴
             ),
             grid.params=list(
               vline=T
             )
  ...
  geom_fruit(data=group,geom=geom_tile,
             ...
             pwidth=0.3,offset=-0.01)+
  scale_fill_manual(values = default_colors <- hue_pal(l=30)(20))+
...
#heatmap_KEGG(project=PROJECT,EXP=geo_exp,DEG=df)
