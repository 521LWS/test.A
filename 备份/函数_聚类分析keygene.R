...
go_KEGG<- function(project,DEG,hu_man, human_db,keygene) {
  ...
  # 2.1 进行GO项的GSEA
  Go_gseresult <- gseGO(geneList, human_db, keyType = "ENTREZID", ont = "all", nPerm = 1000000, minGSSize = 10,maxGSSize = 1000, pvalueCutoff = 1)
  ...
  Go_gseresult@result<-Go_gseresult@result[order(abs(Go_gseresult@result$rank),decreasing = TRUE), ]
  ...
  # 2. 计算富集结果
  #2. 2 进行KEGG通路的GSEA
  KEGG_gseresult <- gseKEGG(geneList,organism = hu_man,exponent = 1.5, nPerm = 1000000,maxGSSize = 1000, pvalueCutoff =1)
...
print(Go4)
  ...
  pdf(paste0(project,"/",project, "-","GSEA_KEGG_gseresult2.pdf"), width = 10, height = 8)
  ...
  Go_Reactomeresult <- gsePathway(geneList,organism = hu_man,exponent = 1.5, nPerm = 1000000, minGSSize = 10, maxGSSize = 1000, pvalueCutoff = 1)
  ...
  Go_Reactomeresult@result<-Go_Reactomeresult@result[order(abs(Go_Reactomeresult@result$rank),decreasing = TRUE), ]
