...
# 2.1 进行GO项的GSEA
  Go_gseresult <- gseGO(geneList, human_db, keyType = "ENTREZID", ont = "all", nPerm = 100000, minGSSize =10,maxGSSize = 1000, pvalueCutoff = 1)
  Go_gseresult<- setReadable(Go_gseresult,OrgDb=human_db,keyType ='ENTREZID')
  

  Go_gseresult@result<- subset(Go_gseresult@result,abs(NES) >= 1 & pvalue< 0.05)
  ...
  Go_gseresult@result<-Go_gseresult@result[order(Go_gseresult@result$ONTOLOGY ,decreasing = FALSE), ]
...
p1<-GOBubble(circ,1:20,display = 'multiple')

p2<-GOBubble(circ,labels = 1)
print(p1)
print(p2)
dev.off()

# 使用 chord_dat() 函数创建 chord 图所需的数据
pdf(paste0(project,"/",project, "-","Go_弦图.pdf"), width = 12, height = 8)
chord <- NULL
...
# 可秀图
pdf(paste0(project,"/",project, "-","Go_聚类图.pdf"), width = 12, height = 8)
...
# 2. 计算富集结果
#2. 2 进行KEGG通路的GSEA
KEGG_gseresult <- gseKEGG(geneList,organism = hu_man,exponent = 1.5, nPerm = 1000000,minGSSize = 10,maxGSSize = 1000, pvalueCutoff =1)
...
KEGG_gseresult@result<-KEGG_gseresult@result[order(abs(KEGG_gseresult@result$rank),decreasing = TRUE), ]
...
# 改为色块儿图例
library(RColorBrewer)
mypalette <- colorRampPalette(c("blue","white","red" ))(30)
p1<-ggplot(data, aes(x = count/length(geneList), y = reorder(Description, count/length(geneList)))) +
  geom_point(aes(color = enrichmentScore, size = count), shape = 16, stroke = 1, alpha = 0.7) +
  scale_size_continuous(range=c(2,12))+
  scale_colour_gradientn(colors = mypalette, breaks = seq(min((data$enrichmentScore)), max((data$enrichmentScore)), length.out = 20)) +
  theme_minimal() + # 改为色块儿图例
  labs(x = "geneRatio", y = "Pathway") +
  guides(
    color = guide_colorsteps(title = "enrichmentScore")  # 手动指定图例中的刻度标签数量
  ) +# 将图例改为渐变色块形式
  theme(
    axis.line = element_line(color = "black", size = 0.5, linetype = "solid"),
    axis.ticks = element_line(color = "black", size = 0.25, linetype = "solid"),
